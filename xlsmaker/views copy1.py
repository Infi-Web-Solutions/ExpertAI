...
def extrair_colunas_grafico(user_input, colunas_disponiveis):
    ...
    padroes = [
        ...
        r'(?:quero(?: um)?|gera(?:-me)?|cria(?:-me)?|plota(?:-me)?)\s+(?:ver\s+)?(?:um\s+)?gr[aá]fico(?: que mostra)?\s+(.*?)\s+(?:por|ao longo de|contra|vs|versus|em fun[cç]ão de)\s+(.*?)(?:\.|$)'
        ...
        r'(?:quero(?: um)?|gera(?:-me)?|cria(?:-me)?)\s+gr[aá]fico(?: que mostra)?\s+(.*?)\s+para cada\s+(.*?)(?:\.|$)',    
    ]
...
def corrigir_formula_erro_ia(formula: str, idioma='en', abas_existentes=None, colunas_por_aba=None, nome_aba_atual=None) -> str:
    ...
    # Primeiro corrige SUMIFS/SOMASES para evitar conflito com VLOOKUP
    if idioma.startswith('pt'):
        ...
        formula = re.sub(
            r"SOMASE\(([^,;]+)[,;]([^,;]+)[,;]([^)]+)\)\s*-\s*SOMASE\(([^,;]+)[,;]([^,;]+)[,;]([^)]+)\)",
            r"SOMASES(\3{separator}\1{separator}\2)-SOMASES(\6{separator}\4{separator}\5)",
            formula
        )
    else:
    ...
    return tradutor.corrigir_formulas_comuns(
        ...
        nome_aba_atual=nome_aba_atual
    )
    
def ensure_formulas_valid(df):
    """Remove aspas externas de fórmulas mantendo aspas internas necessárias"""
    for col in df.columns:
        df[col] = df[col].apply(
...
def final_validation(df):
    """Ultimate check before Excel export"""
    for col in df.columns:
        df[col] = df[col].apply(
...
def generate_spreadsheet_view(request):
    ...
    prompt_template = PromptTemplate.from_template(
        ...
        Você é uma IA especialista em Excel avançado. Sua tarefa é gerar uma planilha completa com base na descrição fornecida. A planilha deve ser estruturada profissionalmente, conter dados realistas, fórmulas funcionais e, quando solicitado, dashboards e segmentações interativas.

        ---
        Instruções:

        1. **Formato de Saída**
        - Use blocos iniciados por `Sheet: NOME_DA_ABA` para representar múltiplas abas
        - Cada aba deve ser gerada como **CSV com cabeçalho na primeira linha**
        - Separe planilhas com uma linha em branco

        2. **Idioma**
        - O idioma das fórmulas deve ser: ****
        - Utilize:
            - PT: `SE`, `MÉDIA`, `SOMASE`, `ÍNDICE`, `CORRESP`, etc. com `;`
            - EN: `IF`, `AVERAGE`, `SUMIF`, `INDEX`, `MATCH`, etc. com `,`
        - Nunca misture idiomas
        - Exemplo PT: `=SE(B2>10;"Sim";"Não")`
        - Exemplo EN: `=IF(B2>10,"Yes","No")`

        3. **Dados**
        - Gere **exatamente 10 registros realistas por aba**
        - Os dados devem ser coerentes entre si (ex: Vendas ↔ Produtos)
        - Inclua todas as colunas solicitadas

        4. **Fórmulas**
        - Utilize **referências relativas** (ex: `B2`, `C3`)
        - Use separadores corretos de acordo com o idioma
        - Não envolva fórmulas com aspas desnecessárias
        - Para fórmulas de matriz: use prefixo `@` (caso aplique)
        - Exemplos:
            - PT: `=SOMASE(Vendas!D:D;A2;Vendas!G:G)`
            - EN: `=SUMIF(Sales!D:D,A2,Sales!G:G)`

        5. **Relacionamento entre Abas**
        - Quando aplicável, conecte dados entre abas usando:
            - PT: `PROCV`, `ÍNDICE`, `CORRESP`
            - EN: `XLOOKUP`, `INDEX`, `MATCH`

        6. **Recursos Avançados (opcional)**
        - Somente adicione se mencionado pelo usuário
        - Pode incluir:
            - Aba com **tabela dinâmica resumida**
            - Aba com **dashboard visual** com:
                - Gráficos (colunas, pizza, barras, etc.)
                - Filtros (ex: Mês, Região, Produto)
                - Indicadores (KPIs como Receita, Meta, Ticket Médio)

        7. **Gráficos (simulação textual)**
        ...
        Sheet: Vendas
        ID Venda,Data,Cliente,Produto,Quantidade,Preço Unitário,Total
        001,01/04/2024,Carlos,Notebook,2,3500,"=E2*F2"

        Sheet: Produtos
        Produto,Categoria,Estoque
        Notebook,Informática,50

        Sheet: Resumo
        Produto,Total Vendido,Receita Total
        Notebook,"=SOMASE(Vendas!D:D;A2;Vendas!E:E)","=SOMASE(Vendas!D:D;A2;Vendas!G:G)"
        ...
        ---
        Tarefa: Gere a estrutura da planilha conforme as instruções acima, preenchendo com dados coerentes, fórmulas funcionais e organização perfeita por abas.
        """
    )
    ...
    if request.method == 'POST':
        try:           
            ...
            validator = FormulaValidator(excel_language)
            processed_sheets = []
            abas_existentes = [sheet['name'] for sheet in sheets if 'name' in sheet]
            
            # Primeiro coletamos todas as colunas de todas as abas
            colunas_por_aba = {}
            for sheet in sheets:
                if sheet['data']:
                    df_sheet = parse_sheet_data('\n'.join(sheet['data']), excel_language)          
                    colunas_por_aba[sheet['name']] = df_sheet.columns.tolist()
            ...
            for sheet in sheets:
                ...
                # Aplicar validação em todas as células
                for col in df.columns:
                    df[col] = df[col].apply(
                        lambda x: validator.validate_formula(
                            corrigir_formula_erro_ia(
                                str(x),
                                idioma=excel_language,
                                abas_existentes=abas_existentes,
                                colunas_por_aba=colunas_por_aba,
                                nome_aba_atual=sheet['name']
                            ) if isinstance(x, str) and x.startswith('=') else x
                        )
                    )
                ...
                processed_sheets.append((sheet['name'], df))
            ...
            if not processed_sheets:
                ...
                if not df.empty:
                    ...
                    colunas_por_aba = {'Planilha Principal': df.columns.tolist()}
                    
                    for col in df.columns:
                        df[col] = df[col].apply(
                            lambda x: corrigir_formula_erro_ia(
                                str(x),
                                excel_language,
                                abas_existentes,
                                colunas_por_aba,
            ...
            # Gerar arquivo Excel
            # Aplicar antes de gerar o Excel
            processed_sheets = [(name, final_validation(df)) for name, df in processed_sheets]
            buffer = BytesIO()
...
def upload_planilha(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # prompt = resposta_ia.format(user_input=descricao)
            # response = model.invoke([HumanMessage(content=prompt)])
            # print(response.content)
